...
(function (global) {
  ...
  const USE_CASES = [
    ...
    // ---------------- M3-UC2 Six-Pillar Stock Analysis ----------------
    {
      ...
      tour: [
        { fr: 'FR-FA-05/06', priority: 'Must', status: 'partial', selector: '#panel-m3uc2-fundamental', requirement: 'DuPont decomposition + forensic accounting scores (Beneish/Sloan/Altman/Piotroski).', achieved: 'All four formulas computed exactly as specified, but the spec says these should be *sourced from Module 1* — Module 1 isn\'t built in this prototype, so they\'re computed directly here on synthetic financials instead.' },
        { fr: 'FR-FA-11/12', priority: 'Must', status: 'partial', selector: '#panel-m3uc2-fundamental', requirement: '64 sector KPIs across 12 sectors + peer comparison.', achieved: '4 illustrative KPIs and same-sector peer ROE/growth shown — a representative subset, not the full 64-KPI library.' },
        { fr: 'FR-TA-01..04', priority: 'Must', status: 'full', selector: '#panel-m3uc2-technical', requirement: 'MA/RSI/MACD/Bollinger indicator states.', achieved: 'All four computed with real formulas from the stock\'s OHLCV history.' },
    ...
    // ---------------- M3-UC3 Risk & Quantitative Analytics ----------------
    {
      ...
      tour: [
        { fr: 'FR-RQ-01', priority: 'Must', status: 'partial', selector: '#panel-m3uc3-mc', requirement: '≥10,000 GBM/bootstrapped paths with CI bands and P(target hit).', achieved: 'Real GBM simulation with CI percentiles and target-hit probability; default path count is 2,000 for prototype responsiveness (adjustable up to 10,000+ in the form).' },
        ...
        { fr: 'FR-RQ-02', priority: 'Must', status: 'full', selector: '#panel-m3uc3-adversarial', requirement: 'Bull/Bear adversarial engine with documented convergence.', achieved: 'Deterministic rule-based scoring over the six pillars, with a reproducible convergence-proof string — matches the "not free-form" constraint.' },
        { fr: 'FR-RQ-03/04', priority: 'Must', status: 'full', selector: '#panel-m3uc3-stress', requirement: 'Red-team + black-swan stress checks with probability-weighted tail impact.', achieved: '5 scenarios (3 red-team, 2 devils-advocate) each with a probability and shock magnitude, producing a stressed-conviction and weighted tail-impact figure.' },
        ...
        { fr: 'FR-RQ-08', priority: 'Must', status: 'full', selector: '#panel-m3uc3-inference', requirement: 'Inference map: dependency graph of conviction derivation.', achieved: 'Rendered as a node/edge table (six pillars + adversarial stance, each feeding the conviction node) rather than a graphical diagram — same information, table form.' },
    ...
    // ---------------- M3-UC5 Synthesis & Conviction Score ----------------
    {
      ...
      tour: [
        { fr: 'FR-CS-01', priority: 'Must', status: 'full', selector: '#panel-m3uc5-score', requirement: 'Composite 0-100 Conviction Score mapped to a 5-tier rating.', achieved: 'Weighted sum of the six pillar sub-scores (weights re-normalised to 1) mapped to Strong Buy...Strong Sell bands.' },
        ...
        { fr: 'FR-CS-04', priority: 'Must', status: 'full', selector: '#panel-m3uc5-ledger', requirement: 'Immutable, tamper-evident, timestamped ledger + time-series tracking vs OHLC.', achieved: 'Real SHA-256 hash chain (each entry hashes the previous entry\'s hash); a break-detection check runs on every render. Timeline computes realised performance from price-at-call vs current price.' },
        { fr: 'FR-CS-05', priority: 'Must', status: 'full', selector: '#panel-m3uc5-sandbox', requirement: 'Scenario sandbox: adjust macro, see score/value impact, never published.', achieved: 'Sandbox recomputes the macro pillar and conviction score from your deltas; result is explicitly flagged exploratory/unpublished and never written to the ledger.' },
    ...
    // ---------------- M3-UC6 Screening & Discovery ----------------
    {
      ...
      tour: [
        { fr: 'FR-SC-01', priority: 'Must', status: 'full', selector: '#panel-m3uc6-results', requirement: 'Screener with CMP, target, upside, ROI, filterable by conviction/sector/cap.', achieved: 'Live-computed CMP/ROE/target from the universe; filterable via bucket or NL query.' },
    ...
    // ---------------- M3-UC7 Personalised Recommendation Layer ----------------
    {
      ...
      render(container, data) {
        ...
        if (!data.switchSuggestions.eligible) {
          p2.appendChild(el('div', 'empty-hint', data.switchSuggestions.reason));
      ...
      tour: [
        ...
        { fr: 'FR-PR-04', priority: 'Must', status: 'full', selector: '#panel-m3uc7-gap', requirement: 'Model-portfolio templates with gap analysis.', achieved: '3 governed cap-bucket templates (Conservative/Balanced/Aggressive) benchmarked against your actual current-value mix.' },
